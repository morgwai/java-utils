...
public class AwaitableTest {
	...
	public void testAwaitableOfMonitorCondition() throws Throwable {
		...
		final var awaitingThread = new Thread(() -> {
			try {
				assertTrue(
					"awaiting should succeed",
					Awaitable.ofMonitorCondition(monitor, () -> conditionHolder[0]).await(100L)
				);
			} catch (Throwable e) {
				asyncError = e;
			}
	...
	public void testAwaitableOfMonitorConditionWithInterrupt() throws Throwable {
		...
		final var awaitingThread = new Thread(() -> {
			try {
				Awaitable.ofMonitorCondition(monitor, () -> false).await(0L);
		...
		final var awaitingThread = new Thread(() -> {
			try {
				assertFalse("awaiting should fail",
					Awaitable.ofMonitorCondition(monitor, () -> false).await(20L));
			} catch (Throwable e) {
				asyncError = e;
			}
		});
